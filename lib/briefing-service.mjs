...
export const KIS_ENV = (process.env.KIS_ENV || "prod").trim().toLowerCase();
...
export async function buildBriefingResponse(requestedSymbols) {
  const { entries: selectedSymbols, unknownSymbols } = normalizeSymbols(requestedSymbols);
  ...
  if (!hasKisCredentials()) {
    return {
      ...
      message: "KIS 환경변수가 없어 샘플 데이터로 브리핑했습니다.",
      updatedAt: new Date().toISOString(),
      unknownSymbols,
...
function normalizeSymbols(requestedSymbols) {
  ...
  for (const rawKeyword of requestedSymbols) {
    ...
    const entry = findSymbolEntry(keyword);

    if (!entry) {
      unknownSymbols.push(keyword);
      continue;
    }
